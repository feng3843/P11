...
@interface LRNavTabBar () <LRPopViewDelegate>
{
    ...
    LRPopView       *_popView;              // when item menu, will show this view
    
    NSMutableArray  *_items;                // LRNavTabBar pressed item
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(40, NAV_TAB_BAR_HEIGHT - 3.0f, 28, 2.0f)];
    //_line.backgroundColor = UIColorWithRGBA(20.0f, 80.0f, 200.0f, 0.7f);
    _line.backgroundColor = [UIColor colorWithHexString:@"ffffff"];;
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        button.frame = CGRectMake(buttonX + 20, DOT_COORDINATE, [widths[index] floatValue], NAV_TAB_BAR_HEIGHT);
//        button.backgroundColor = [UIColor redColor];
        [button setTitle:_itemTitles[index] forState:UIControlStateNormal];
        // 设置按钮的字体颜色
        [button setTitleColor:[UIColor colorWithHexString:@"dfdfdf"] forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont boldSystemFontOfSize:14];
        [button addTarget:self action:@selector(itemPressed:) forControlEvents:UIControlEventTouchUpInside];
        [_navgationTabBar addSubview:button];
        
        [_items addObject:button];
        buttonX += [widths[index] floatValue]+ 20;
    }
    ...
    if (pop)
    {
        ...
        [UIView animateWithDuration:0.5f animations:^{
            ...
            [UIView animateWithDuration:0.2f animations:^{
                if (!_popView)
                {
                    _popView = [[LRPopView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, NAVIGATION_BAR_HEIGHT, SCREEN_WIDTH, self.frame.size.height - NAVIGATION_BAR_HEIGHT)];
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        _line.frame = CGRectMake(button.frame.origin.x + 20.0f, _line.frame.origin.y, 28, _line.frame.size.height);
    }];
}
