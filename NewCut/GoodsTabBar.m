...
#import "GoodsPopView.h"
#import "PYAllCommon.h"

@interface GoodsTabBar () <GoodsPopViewDelegate>
{
    ...
    GoodsPopView       *_popView;              // when item menu, will show this view
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(10 , 36 - 5.0f, 29, 1.5f)];
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    for (NSString *title in titles)
    {
        ...
        NSNumber *width = [NSNumber numberWithFloat:size.width + 24.0f];
    ...
    if (pop)
    {
        ...
        [UIView animateWithDuration:0.5f animations:^{
            ...
            [UIView animateWithDuration:0.2f animations:^{
                if (!_popView)
                {
                    _popView = [[GoodsPopView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, 0, SCREEN_WIDTH, self.frame.size.height - NAVIGATION_BAR_HEIGHT)];
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        _line.frame = CGRectMake(button.frame.origin.x +12.0f, _line.frame.origin.y, 29, _line.frame.size.height);
        //        button.selected = YES;
